...
#### 1. PASS 운전면허 확인 서비스 연동(Kotlin, Spring)
PASS 앱을 이용한 면허 검증 시스템 구축

- 면허 정보 암호화를 위한 대칭키 교환을 코루틴을 통한 비동기 방식으로 구현

#### 2. 모바일 신분증(운전면허증) 연동(Kotlin, Spring)
행정안전부 모바일 신분증 표준 프로토콜 준수하여 면허 검증 시스템 구축

- 면허 정보 조회·갱신 서버와 면허 인증 서버 간 역할을 분리하여 로직을 독립 관리하고 확장 가능한 구조로 설계
- 인증 거래 테이블 설계 시 요청 서비스·유저 식별키를 포함하여 다른 내부 서비스에서도 재사용 가능한 범용 구조로 구현
- 연동 외부 SDK를 어댑터 패턴으로 적용

**성과**: 면허 등록 사용성 관련 VOC 52.3% 감소

---

#### 3. 메일 발송 서비스 개선(Kotlin, Spring, AWS SES)
기존 Blocking 기반 메일 발송 시스템의 응답 지연 및 운영 리스크를 해결하기 위한 구조 전면 개선

- Blocking → Non-Blocking 전환
    - RestTemplate / SesClient → WebClient / SesAsyncClient + Coroutine 기반 비동기 구조로 재설계
- 이메일 유효성 검증 정책 고도화
    - 검증 결과를 암호화하여 DB 저장함으로써 개인정보 보호와 재검증 여부 판단 동시 확보
    - 시간 기반 재검증 정책 도입으로 불필요한 중복 검증 제거
- AWS SES Bounce 이벤트를 SQS 기반 비동기 처리로 연동하여 검증 결과·발송 이력 자동 갱신
- 순차 처리 배치 요청을 병렬 처리 구조로 전환하여 대량 요청 시에도 전체 처리 시간 안정화
- 인증 API 할당량 초과 시 by-pass 로직 적용 및 모니터링·알람 체계 구축

**성과**:
- 응답 속도 개선: 외부 인증 API 미사용 시 메일 발송 구간 응답 속도 **500ms 이하** (기존 1초 이상 소요)
- 비용 절감: 외부 인증 API 사용량 기존 대비 약 **80% 절감** (일일 1,000건 미만 유지)
- 운영 안정성: 할당량 초과 사전 감지 및 by-pass 적용으로 서비스 장애 확산 최소화

---

#### 4. 마케팅 수신 동의 데이터 보정(Kotlin, Spring Batch)
누락된 마케팅 수신 동의 데이터를 동기화

- 누락된 사용자 마케팅 수신 동의 데이터 **2,500만 건**을 Spring Batch로 전송
- 대량 데이터 전송 시 내결함성 확보: executionContext에 마지막 업데이트 데이터 고유키를 저장·활용하여 컨텍스트 유지

**성과**: 마케팅에 사용할 유저를 식별하는 데 올바른 최신 데이터를 사용하도록 개선하여 잘못된 마케팅 수신을 전달받는 유저 감소
...
- 클러스터링 대상 요청 TPS: 평균 1.3k TPS, 피크 1.7k TPS
...
- **인프라 및 도구:** Kubernetes, ArgoCD, Docker, AWS, Git, CI/CD (GitHub Actions)
